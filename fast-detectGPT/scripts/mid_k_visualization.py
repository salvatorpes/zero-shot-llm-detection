...
def create_perplexity_line_plots(results_subdir=None):
    ...
    ax1.set_title('Mean Sampled Perplexity vs Num of Pos (Lines by Cutoff-K)', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Num of Pos', fontsize=12)
    ax1.set_ylabel('Mean Sampled Perplexity', fontsize=12)
    ax1.grid(True, alpha=0.3)

    for i, limit in enumerate(unique_limits):
        positions = [item[0] for item in data_by_limit[limit]]
        sample_values = [item[1] for item in data_by_limit[limit]]
        ax1.plot(positions, sample_values, marker='o', linewidth=2, markersize=6,
    ...
    ax1.set_ylim(min_sample - sample_margin, max_sample + sample_margin)
    ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
