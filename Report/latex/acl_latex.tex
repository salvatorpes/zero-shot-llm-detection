...
% DO NOT DELETE. WAITING FOR FULL RESULTS.
...
%     & XSum & HC3 & \textbf{Avg.} \\
%     \midrule
%     Likelihood & 0.1848 & 0.0000 & 0.0000 \\
...
% Table~\ref{tab:API_results} reports the AUROC scores for the GPT-4o mini API experiments. Unlike the open-weight models, GPT-4o mini exhibits unstable behavior: Fast-DetectGPT performs relatively poorly, while both Likelihood and LogRank fail entirely. In contrast, the Entropy baseline achieves near-perfect accuracy across both datasets, which is an unexpected outcome given its weak performance in previous experiments.

% This unexpected outcome is likely due to the API’s limitation of exposing only the top 20 token probabilities, which distorts the probability distribution seen by the detector. As a result, Fast-DetectGPT’s curvature-based statistic becomes unreliable, while entropy, which is being calculated from a smaller but consistent portion of the probability mass, remains informative.
